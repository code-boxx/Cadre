...
define("I_NAME", "CADRE"); // PROJECT NAME
...
if ($_PHASE=="B") {
  // (A7) FILES & FOLDERS TO CHECK FOR READ/WRITE PERMISSIONS
  // @TODO - ADD MORE AS NECESSARY
  define("I_ALL", [
  ...
  // (A8) DATABASE DEFAULTS
  // @TODO - SET YOUR OWN
  define("I_DB_HOST", "localhost");
  define("I_DB_NAME", "cadre");
  define("I_DB_USER", "root");
  define("I_DB_PASS", "");

  // (A9) APACHE + PHP VERSION + EXTENSIONS
  ...
  // (A11) PUSH NOTIFICATION VAPID KEYS
  if (I_PUSH && I_OPENSSL) {
...
if ($_PHASE == "B") {
  ...
  // (B4) FILES & FOLDERS - READ/WRITE PERMISSIONS
  foreach (I_ALL as $p) {
...
<html>
  <head>
    ...
    *{font-family:arial,sans-serif;box-sizing:border-box}body{padding:20px;max-width:600px;margin:0 auto;background:#f4f4f4}
    .danger{padding:15px;margin-bottom:15px;background:#bb2323;color:#fff;font-weight:700}
    ...
    #iHead{display:flex;align-items:center;margin-bottom:50px}#iHead h1{margin:0}#iHead img{margin-right:20px}
    h2{font-size:24px;margin:0 0 10px 0}.iSec{background:#fff;border:1px solid #dbdbdb;padding:20px;margin-bottom:20px}
    select,input,label{display:block;width:100%}select,input{font-size:16px;padding:10px}
    label{font-size:18px;color:#ff2424;text-transform:uppercase;padding:10px 0}
    ...
    function install () {
      ...
      // (D3-3) PASSWORD STRENGTH CHECK - AT LEAST 8 CHARACTERS ALPHANUMERIC
      ...
      .then(res => { if (res.status!=200) {
        alert(`SERVER ${res.status} ERROR - Are you sure the host URL is correct?`);
        console.error(res);
  ...
  <body>
    ...
    <div class="danger">
      The installer cannot verify if you are running Apache Web Server,
      but you can still proceed if you want.
    </div>
    <?php } ?>
    <?php if (I_REWRITE === false) { ?>
    <div class="danger">
      If you are using Apache, please make sure <code>MOD_REWRITE</code> is enabled.
      If not, you will need to "translate" <code>/.htaccess</code> on your own.
    </div>
    <?php } ?>
...
if ($_PHASE == "E") {
  ...
  // (E2) CREATE DATABASE + IMPORT SQL FILE(S)
  try {
  ...
  if (I_USER) { try {
    $stmt = $pdo->prepare("REPLACE INTO `users` (`user_name`, `user_email`, `user_password`, `user_title`, `user_level`) VALUES (?,?,?,'Administrator','A')");
  ...
  // (E5) CORE_CONFIG.PHP SETTINGS TO UPDATE
